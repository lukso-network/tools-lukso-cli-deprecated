...
// depositRangeCmd represents the deposit command
var depositRangeCmd = &cobra.Command{
	Use:   "range",
...
func init() {
	validatorCmd.AddCommand(depositRangeCmd)
	depositRangeCmd.Flags().BoolP(CommandOptionDry, "d", false, "don't run the transactions but just prepare it")
}
